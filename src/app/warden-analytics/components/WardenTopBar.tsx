...
import { LogOut, Bell, Download, RefreshCw, BookOpen, Settings, Monitor } from 'lucide-react';
...
export default function WardenTopBar({ onExportClick, onRefreshClick }: WardenTopBarProps) {
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      console.log('Failed to load warden notifications:', error);
    }
  }, []);
    ...
    <div className="py-3.5 sm:py-4 flex flex-col gap-2.5 lg:flex-row lg:items-center lg:justify-between">
      <div className="min-w-0">
        <div className="flex items-center gap-2.5 mb-1">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-[0.9rem] gradient-primary flex items-center justify-center shadow-[0_0_12px_rgba(124,58,237,0.14)]">
            <span className="text-lg">🛡️</span>
          </div>
          <div>
            <h1 className="text-base sm:text-lg lg:text-xl font-bold text-white leading-tight">Warden Analytics</h1>
            <p className="text-[11px] sm:text-xs lg:text-sm text-[hsl(var(--muted-foreground))]">
              {displayName} · Hostel {displayHostel} Warden
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          <span className="text-[10px] sm:text-[11px] bg-violet-500/15 text-violet-400 border border-violet-500/25 px-2.5 py-0.5 rounded-full flex items-center gap-1">
          ...
          <span className="text-[10px] sm:text-[11px] bg-green-500/15 text-green-400 border border-green-500/25 px-2.5 py-0.5 rounded-full">
      ...
      <div className="flex items-center gap-2 flex-wrap lg:justify-end">
        ...
        <button onClick={onExportClick} className="btn-glass hidden sm:inline-flex items-center gap-2 text-sm bg-gradient-to-r from-emerald-600/20 to-indigo-600/20 border-emerald-400/30">
          <Download size={14} />
          <span>Export Report</span>
        </button>
        <button onClick={onRefreshClick} className="btn-glass hidden sm:inline-flex items-center gap-2 text-sm">
          <RefreshCw size={14} />
          <span>Refresh</span>
        </button>
